...
{
  printer *printer = userdata;
	
  return printer_pretty(printer, type, data, length);
}
...
static int do_verify(json_config *config, const char *filename)
{
  ...
  ret = init_parser(&parser, config, NULL, NULL);
  if (ret) {
...
static int do_format(json_config *config, const char *filename, const char *outputfile)
{
  ...
  ret = init_printer(&printer, printchannel, stdout);
  if (ret) {
  ...
  ret = init_parser(&parser, config, &prettyprint, &printer);
  if (ret) {
    fprintf(stderr, "error: initializing parser failed: [code=%d] %s\n", ret, string_of_errors[ret]);
    return ret;
  }
