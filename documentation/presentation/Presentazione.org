...
#+ATTR_REVEAL: :frag (appear)
In seguito alla registrazione per accedere alla piattaforma dovranno scansionare un opportuno ~QR-Code~, generato con cadenza giornaliera sfruttando l'utility [[https://github.com/fukuchi/libqrencode][qrencode]], utilizzato come chiave di autenticazione durante lo scambio di messaggi tra
